...
if supports_scc; then
    ...
    if command -v oc >/dev/null 2>&1 ; then
        ...
        oc adm policy remove-scc-from-group ibm-seas-scc system:serviceaccounts:$namespace
    else
...
if supports_psp; then
    if [ "$isApplied" == "false" ]; then
		...
		sed 's/{{ NAMESPACE }}/'$namespace'/g' ../../pre-install/namespaceAdministration/ibm-seas-rb.yaml > $namespace-ibm-seas-rb.yaml

		# Delete the role binding for all service accounts in the current namespace
		kubectl delete -f $namespace-ibm-seas-rb.yaml -n $namespace
