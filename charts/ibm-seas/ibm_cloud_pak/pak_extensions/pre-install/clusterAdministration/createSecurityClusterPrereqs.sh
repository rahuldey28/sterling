...
if supports_scc; then
  ...
  kubectl create -f ibm-seas-scc.yaml --validate=false
  echo "Creating Cluster Role"
  kubectl create -f ibm-seas-cr-scc.yaml --validate=false
  isApplied=true
fi
...
if supports_psp; then
  ...
  if [ "$isApplied" == "false" ]; then
    ...
    kubectl create -f ibm-seas-psp.yaml
    echo "Creating Cluster Role"
    kubectl create -f ibm-seas-cr.yaml
  fi
fi
